...
function wheelColumn(unit, values, selected) {
  return `<div class="time-wheel__column" data-wheel-column data-wheel-unit="${unit}" aria-label="${unit === "hour" ? "Hora" : "Minuto"}"><span class="time-wheel__fade time-wheel__fade--top"></span>${values.map((value) => `<button type="button" class="time-wheel__option ${value === selected ? "is-selected" : ""}" data-wheel-value="${value}" aria-pressed="${value === selected}">${String(value).padStart(2, "0")}</button>`).join("")}<span class="time-wheel__fade time-wheel__fade--bottom"></span></div>`;
}

function timeWheel(name, value) {
  const { hour, minute } = timeParts(value);
  return `<div class="time-wheel" data-time-wheel data-time-name="${name}"><input type="hidden" name="${name}" value="${displayTime(value)}" />${wheelColumn("hour", HOURS, hour)}<span class="time-wheel__divider">:</span>${wheelColumn("minute", MINUTES, minute)}</div>`;
}

function scheduleFields(values, disciplines) {
  return `<div class="schedule-fields"><label class="field"><span>Disciplina</span><span class="field__control">${icon("book", 17)}<select name="disciplineId" required><option value="" disabled ${!values.disciplineId ? "selected" : ""}>Selecione uma disciplina</option>${disciplines.map((discipline) => `<option value="${escapeHtml(discipline.id)}" ${discipline.id === values.disciplineId ? "selected" : ""}>${escapeHtml(discipline.nome_disciplina)}</option>`).join("")}</select></span></label><label class="field"><span>Dia da semana</span><span class="field__control">${icon("calendar", 17)}<select name="weekday" required>${WEEKDAY_NAMES.map((day, index) => `<option value="${index}" ${String(index) === values.weekday ? "selected" : ""}>${day}</option>`).join("")}</select></span></label><div class="schedule-time-fields"><label class="field"><span>In\u00edcio</span>${timeWheel("startTime", values.startTime)}</label><label class="field"><span>Fim</span>${timeWheel("endTime", values.endTime)}</label></div></div>`;
}

function scheduleEditorModal(schedule, disciplines) {
  const editing = Boolean(schedule);
  return `<div class="modal-backdrop" data-schedule-editor-backdrop><section class="modal modal--schedule-editor" role="dialog" aria-modal="true" aria-labelledby="schedule-editor-title"><form class="schedule-editor" data-schedule-editor novalidate><div class="schedule-editor__head"><div><span class="eyebrow">${editing ? "EDITAR HOR\u00c1RIO" : "NOVO HOR\u00c1RIO"}</span><h2 id="schedule-editor-title">${editing ? "Ajuste sua aula" : "Adicionar aula \u00e0 grade"}</h2><p>Use as roletas para selecionar os hor\u00e1rios em intervalos de cinco minutos.</p></div><button class="icon-button" type="button" data-close-schedule-editor aria-label="Fechar">${icon("close", 19)}</button></div>${scheduleFields(scheduleValues(schedule), disciplines)}<div class="schedule-editor__actions">${editing ? `<button class="button button--danger" type="button" data-delete-schedule>${icon("trash", 16)} Excluir</button>` : ""}<span></span><button class="button button--ghost" type="button" data-close-schedule-editor>Cancelar</button><button class="button button--primary" type="submit">${icon("save", 16)} ${editing ? "Salvar hor\u00e1rio" : "Adicionar \u00e0 grade"}</button></div></form></section></div>`;
}
...
function boardRange(schedules) {
  if (!schedules.length) return { start: 7 * 60, end: 18 * 60 };
  ...
  const start = Math.max(0, Math.floor((earliest - 45) / 60) * 60);
  const end = Math.min(24 * 60, Math.max(start + (5 * 60), Math.ceil((latest + 45) / 60) * 60));
  return { start, end };
}
...
function weeklyBoard(schedules, disciplines, teachers, editing) {
  ...
  const maxHeight = compactScreen ? 320 : 480;
  const minHeight = compactScreen ? 220 : 300;
  const minimumBlockHeight = compactScreen ? 30 : 36;
  const boardHeight = Math.min(maxHeight, Math.max(minHeight, totalMinutes * 0.58));
...
export function schedulesView({ profile, disciplines, teachers, schedules, editing }) {
  ...
  return `<section class="page schedules-page"><div class="page-heading page-heading--row schedules-page__heading"><div><span class="eyebrow">ROTINA SEMANAL</span><h1>Hor\u00e1rios</h1><p>Visualize as aulas de <strong>${course}</strong> durante a semana.</p></div><div class="schedules-page__actions"><button class="button ${editing ? "button--secondary" : "button--primary"}" data-toggle-schedule-edit>${icon(editing ? "check" : "edit", 17)} ${editing ? "Concluir edi\u00e7\u00e3o" : "Modo de edi\u00e7\u00e3o"}</button>${editing ? `<button class="button button--primary" data-add-schedule ${canEdit ? "" : "disabled"}>${icon("plus", 17)} Adicionar aula</button>` : ""}</div></div>${!canEdit ? `<div class="schedule-needs-discipline">${icon("book", 18)} <span>Cadastre ao menos uma disciplina antes de montar a grade.</span></div>` : ""}<section class="schedule-panel ${editing ? "is-editing" : ""}"><div class="schedule-panel__top"><div><strong>Grade semanal</strong><small>${editing ? "Clique em uma aula para editar." : "Ative o modo de edi\u00e7\u00e3o para fazer altera\u00e7\u00f5es."}</small></div><span>${schedules.length} ${schedules.length === 1 ? "aula" : "aulas"}</span></div>${weeklyBoard(schedules, disciplines, teachers, editing)}</section></section>`;
}
...
export function bindSchedules(root, { disciplines, schedules, editing, onToggleEdit, onCreate, onUpdate, onDelete }) {
  ...
  root.querySelector("[data-add-schedule]")?.addEventListener("click", () => { if (disciplines.length) openScheduleEditor(null, disciplines, onCreate); });
  if (!editing) return;
